...
namespace ObjectStore.Test.Sqlite
{
    public class SqliteDatabaseFixture : IDisposable, IDatabaseFixture
    {
        ...
        public SqliteDatabaseFixture()
        {
            ...
            databaseInitializer.RegisterCreateTableStatement(x => x is IAddTableStatement && x.Tablename == "`dbo.ForeignObjectKeyTable`", (x, s) => s + ";INSERT INTO `dbo.ForeignObjectKeyTable` (`Id`, `Value`) VALUES (1, 'Testentry')");
            databaseInitializer.RegisterCreateTableStatement(x => x is IAddTableStatement && x.Tablename == "`dbo.DifferentWritabilityLevels`", (x, s) =>
            {
            ...
            databaseInitializer.RegisterAddFieldStatment(x => x.Statement is IAddTableStatement && (x.Fieldname == "`Updateable`" || x.Fieldname == "`Readonly`") && x.Statement.Tablename == "`dbo.DifferentWritabilityLevels`", (x, s) => s + " DEFAULT 1");
