...
namespace Ausm.ObjectStore.OrMapping
{
    public partial class InheritensObjectProvider<T> : IObjectProvider
    {
        public class QueryProvider : Expressions.IParseAbleQueryProvider
        {
            ...
            public string Parse(Expression expression, Ausm.ObjectStore.OrMapping.Expressions.ParsedExpression parsedExpression)
            {
            ...
            public IQueryable CreateQuery(Expression expression)
            {
                ...
                return Queryable.Create(_objectProvider, expression);
            }
        ...
        internal class QueryContext
        {
                            ...
                            ((Expression<Func<T, TKey>>)ExpressionHelper.ReplaceExpressionParts(_expression, x =>
                            {
                                if (x.NodeType != ExpressionType.MemberAccess ||
                                    ...
                                    return System.Linq.Expressions.Expression.Condition(
                                        ...
                                        System.Linq.Expressions.Expression.Constant(x.Type.IsValueType ? Activator.CreateInstance(x.Type) : null, x.Type),
                                        x);
                            })).Compile() : _compiled;
                    }
                }
            ...
            public virtual void PrepareSelectCommand(IModifyableCommandBuilder commandBuilder, Ausm.ObjectStore.OrMapping.Expressions.ParsedExpression parentParsedExpression)
            {
            ...
            public IEnumerable<string> PredicateRelatedProperties
            {
                get
                {
                    ...
                    if (_whereExpressions == null || _whereExpressions.Count == 0)
                        return _innerQueryContext == null ? (_predicateRelatedProperties = new List<string>()).AsReadOnly() : (_predicateRelatedProperties = new List<string>(_innerQueryContext.PredicateRelatedProperties)).AsReadOnly();
                        ...
                        IEnumerable<string> enumerable =
                                ExpressionHelper.GetFilteredExpression(expression,
                                    x => x.NodeType == ExpressionType.MemberAccess &&
                                        (x as MemberExpression).Member.MemberType == MemberTypes.Property).Select(x => (x as MemberExpression).Member.Name);
                        if (_predicateRelatedProperties == null)
